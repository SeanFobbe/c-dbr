...
#'      before_body: [C-DBR_Source_TEX_Definitions.tex,C-DBR_Source_TEX_CodebookTitle.tex]
...
#' \begin{itemize}
#' \item Vollständige Aktualisierung der Daten
#' \item Einführung von neuen Variablen für letzte Änderung (Datum), Neufassung (Datum), Aufhebung (Datum jeweils für Verkündung und Wirkung), Lizenz und hierarchische Ketten von Gliederungsbezeichnungen und -titeln
#' \item Parallelisierung der Downloads um Kompilierung des Korpus zu beschleunigen
#' \item Korrektur bei den Dateinamen der Allgemeinen Eisenbahngesetze: GII weist zwei gleichnamige Rechtsakte (\enquote{Allgemeines Eisenbahngesetz}) nach. Beide werden nun mit dem Jahr ihrer Ausfertigung 1951 und 1993 im Langtitel differenziert. In der Vorversion wurde das neuere AEG noch mit dem Jahr 1994 (Inkrafttreten) beschriftet und das andere AEG ohne Jahreszahl.
#' \item Einführung von Netzwerkanalysen (experimentell!)
#' \item Variablen in CSV-Dateien sind nun semantisch sortiert
...
#' \begin{itemize}
#' \item Vollständige Aktualisierung der Daten
#' \item Komplette Überarbeitung des Source Codes
#' \item Erstveröffentlichung eines Codebooks
...
#' \item CSV-Dateien werden durch Parsing der XML-Dateien erstellt
