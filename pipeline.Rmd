...
```{targets tar.enhance.constants.normen}
tar_target(var_constants.normen,
...
```{targets tar.enhance.constants.rechtsakte}
tar_target(var_constants.rechtsakte,
           data.frame(version = as.character(datestamp),
                      doi_concept = config$doi$data$concept,
                      doi_version = config$doi$data$version,
                      lizenz = as.character(config$license$data))[rep(1,
...
values <- data.frame(
    name = c("zip.source", "zip.xml"),
    input = c(quote(files.source), quote(files.xml)),
    suffix.files = c("_Source_Code", "_DE_PDF_Datensatz")
)

zip.all1 <- tarchetypes::tar_map(unlist = FALSE,
                                 values = values,
                                 names = name,
                                 tar_target(zip,
                                            f.tar_zip(x = list(!!!rlang::syms(input)),
                                                      filename = paste0(prefix.files, suffix.files, ".zip"),
                                                      dir = "output",
                                                      mode = "mirror"))
                                 )
