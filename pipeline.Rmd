...
### Korpus der Einzelnormen

```{targets tar.parse1}
tar_target(dt.normen,
           f.dt.einzelnormen(file.xml = files.xml.all[grepl("\\.xml$", files.xml.all)],
                  multicore = config$parallel$parseEinzelnormen,
                  cores = fullCores))

```

### Korpus der Rechtsakte (mit Text) erstellen
...
### Korpus der Rechtsakte (nur Metadaten) erstellen


```{targets tar.parse3}
tar_target(dt.meta,
           f.dt.meta(file.xml = files.xml.all[grepl("\\.xml$", files.xml.all)],
