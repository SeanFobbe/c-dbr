...
Insgesamt werden zusammen mit jeder Kompilierung Dutzende Tests zur Qualitätsprüfung durchgeführt. Alle Ergebnisse der Qualitätsprüfungen sind aggregiert im Compilation Report und im Robustess Checks-Bericht, sowie einzeln im Archiv \enquote{Analyse} zusammen mit dem Datensatz veröffentlicht.
...
## CSV\_Einzelnormen\_ Datensatz 

Die CSV-Datei enthält den Volltext aller Rechtsakte, aufgeteilt in Einzelnormen, sowie alle in diesem Codebook beschriebenen Metadaten. Jede Zeile der CSV-Datei ist eine Einzelnorm. 

Achtung: Enthält nur Rechtsakte, für die mindestens eine Einzelnorm mit Normtext veröffentlicht wurde! Rund 1000 Rechtsakte werden von der Datenquelle nur mit Metadaten (ohne Volltext) bereitgestellt.

**Empfohlen für Legal Tech und quantitative Forschung mit Fokus auf Einzelnormen.**


## CSV\_Einzelnormen\_ Metadaten

Eine CSV-Datei mit Einzelnormen, nur ohne die Normtexte. Sinnvoll für Analyst:innen, die sich nur für die Metadaten interessieren und Speicherplatz sparen wollen. 

Achtung: Enthält nur Informationen zu Rechtsakten, für die mindestens eine Einzelnorm mit Normtext veröffentlicht wurde!


## CSV\_Rechtsakte\_ Datensatz 

Diese CSV-Datei enthält den Volltext aller Rechtsakte, sowie fast alle in diesem Codebook beschriebenen Metadaten. Jede Zeile der CSV-Datei ist ein Rechtsakt.

Die gegenüber den Einzelnormen-Varianten fehlenden Metadaten betreffen vor allem Gliederungsdaten (z.B. Gliederungsüberschrift), die auf Ebene des Rechtsaktes keinen Sinn ergeben. Wurde durch ein Zusammenfügen der Einzelnormen-Variante erstellt. 

Achtung: Enthält nur Rechtsakte, für die mindestens eine Einzelnorm mit Normtext veröffentlicht wurde!

**Empfohlen für Legal Tech und quantitative Forschung mit Fokus auf Rechtsakte.**
 
 
## CSV\_Rechtsakte\_ Metadaten 

Wie die CSV-Datei mit Rechtsakten, nur ohne die Normtexte. Sinnvoll für Analyst:innen, die sich nur für die Metadaten interessieren und Speicherplatz sparen wollen. 
...
Diese CSV-Datei enthält Metadaten für jeden auf der amtlichen Webseite nachgewiesenen Rechtsakt, unabhängig davon, ob mit oder ohne Normtext veröffentlicht. Die Zahl der Rechtsakte ist daher um etwa 1000 höher als bei den anderen CSV-Dateien. Es sind aber keine Normtexte enthalten.
 
 
## XML\_Datensatz 

Die XML-Rohdaten. Alle CSV-Dateien wurden aus diesen Rohdaten extrahiert. XML ist ein komplexes Format und daher nur für entsprechend versierte Forscher:innen geeignet. Sie finden im Codebook eine Konkordanztabelle zwischen den Variablen der CSV-Dateien und der XML-Struktur.
 
 
## XML\_Anlagen 

Manche XML-Dateien verweisen auf Anlagen, meist Bild-Dateien. Diese sind hier zusammengefasst.
 
 
## PDF\_Datensatz 

Die PDF-Dokumente, wie sie vom Bundesjustizministerium auf der amtlichen Webseite bereitgestellt werden, jedoch verbessert durch semantisch hochwertige Dateinamen, die sowohl die Abkürzung, als auch einen modifizierten Langtitel enthalten. 

Die Dateinamen sind so konzipiert, dass sie auch in der traditionellen, qualitativen juristischen Arbeit einen erheblichen Mehrwert bieten. Im Vergleich zu den CSV-Dateien enthalten die Dateinamen nur einen drastisch reduzierten Umfang an Metadaten, um Kompatibilitätsprobleme unter Windows zu vermeiden und die Lesbarkeit zu verbessern. Besonders geeignet für die Arbeit an Desktop PCs.
...
Die Dateinamen sind so konzipiert, dass sie auch für traditionelle qualitative juristische Arbeit einen erheblichen Mehrwert bieten. Im Vergleich zu den CSV-Dateien enthalten die Dateinamen nur einen drastisch reduzierten Umfang an Metadaten, um Kompatibilitätsprobleme unter Windows zu vermeiden und die Lesbarkeit zu verbessern. Besonders geeignet für die Arbeit an mobilen Endgeräten, weil sich das Format der Bildschirmgröße anpassen kann.
 
 
## TXT\_Datensatz 

Diese Variante enthält die vollständigen, aus den PDF-Dateien extrahierten Normtexte der Rechtsakte, aber nur einen drastisch reduzierten Umfang an Metadaten, der dem der PDF-Dateien entspricht. Die TXT-Dateien sind optisch an das Layout der PDF-Dateien angelehnt. 

Geeignet für qualitative Forscher, die nur wenig Speicherplatz oder eine langsame Internetverbindung zur Verfügung haben und für quantitative Forscher, die beim Einlesen der CSV-Dateien Probleme haben.
 
 
##  Netzwerke

Die Gliederungshierarchie aller Rechtsakte wurde in eine Netzwerkstruktur übersetzt und ist in verschiedenen Formaten bereitgestellt (GraphML, Adjazenzmatrizen und Edge Lists). Aus dieser Netzwerkstruktur wurden zudem hierarchische Dendrogramme erstellt um einen visuellen Überblick zu bieten. Einzelnormen sind in den Netzwerkstrukturen aktuell noch nicht berücksichtigt. 

Diese Variante ist noch *experimentell*, sollte also nicht ohne genaue Prüfung für die eigene Forschung verwendet werden.
...
**Geeignet zur Vorbereitung von Lehre und Forschung**
...
## Erläuterung der Kennzahlen
 ...
 Zur besseren Einschätzung des inhaltlichen Umfangs des Korpus dokumentiere ich an dieser Stelle die Verteilung der Werte für verschiedene klassische linguistische Kennzahlen:
...
Sätze & Entsprechen in etwa dem üblichen Verständnis eines Satzes. Die Regeln für die Bestimmung von Satzanfang und Satzende sind in \enquote{Unicode Standard: Annex No 29} beschrieben. Für Rechtsnormen ist diese Zählweise vermutlich nicht robust genug. Interpretieren Sie die Ergebnisse mit großer Vorsicht!\\
...
```{r C-DBR_Einzelnormen_Density_Zeichen, fig.height = 6, fig.width = 9}
...
```{r C-DBR_Rechtsakte_Density_Zeichen, fig.height = 6, fig.width = 9}
...
```{r C-DBR_Einzelnormen_Density_Tokens, fig.height = 6, fig.width = 9}
ggplot(data = dt.normen.final)+
...
```{r C-DBR_Rechtsakte_Density_Tokens, fig.height = 6, fig.width = 9}
ggplot(data = dt.rechtsakte.final)+
    geom_density(aes(x = tokens),
                 fill = "black")+
    scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
...
```{r C-DBR_Einzelnormen_Density_Typen, fig.height = 6, fig.width = 9}
ggplot(data = dt.normen.final)+
...
```{r C-DBR_Rechtsakte_Density_Typen, fig.height = 6, fig.width = 9}
ggplot(data = dt.rechtsakte.final)+
    geom_density(aes(x = typen),
                 fill = "black")+
    scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
...
```{r C-DBR_Einzelnormen_Density_Saetze, fig.height = 6, fig.width = 9}
ggplot(data = dt.normen.final)+
...
```{r C-DBR_Rechtsakte_Density_Saetze, fig.height = 6, fig.width = 9}
ggplot(data = dt.rechtsakte.final)+
    geom_density(aes(x = saetze),
                 fill = "black")+
    scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
...
```{r C-DBR_Einzelnormen_Barplot_Periodikum, fig.height = 10, fig.width = 8}
...
```{r C-DBR_Rechtsakte_Barplot_Periodikum, fig.height = 10, fig.width = 8}
...
```{r C-DBR_Meta_Barplot_Periodikum, fig.height = 10, fig.width = 8}
...
```{r C-DBR_Einzelnormen_Barplot_Ausfertigungsjahr, fig.height = 7, fig.width = 11}
...
```{r C-DBR_Rechtsakte_Barplot_Ausfertigungsjahr, fig.height = 7, fig.width = 11}
...
```{r C-DBR_Meta_Barplot_Ausfertigungsjahr, fig.height = 7, fig.width = 11}
...
```{r, C-DBR_Density_Dateigroessen_XML, fig.height = 6, fig.width = 9}
...
```{r, C-DBR_Density_Dateigroessen_PDF, fig.height = 6, fig.width = 9}
...
```{r, C-DBR_Density_Dateigroessen_TXT, fig.height = 6, fig.width = 9}
...
```{r, C-DBR_Density_Dateigroessen_EPUB, fig.height = 6, fig.width = 9}
